      ...
      <div class="cart-body">
        <ul class="cart-list" v-for="shop in shopcartList" :key="shop.id">
          ...
          <li class="cart-list-con5">
            <a href="javascript:;" class="mins" @click="updateShopNum(shop, -1)"
              >-</a
            >
            <input
              autocomplete="off"
              ...
              minnum="1"
              class="itxt"
              @change="updateShopNum(shop, $event.target.value)"
            />
            <a href="javascript:;" class="plus" @click="updateShopNum(shop, 1)"
              >+</a
            >
          </li>
...
export default {
  ...
  methods: {
    ...
    async updateShopNum(shop, updateNum) {
      if (shop.skuNum + updateNum < 1) {
        updateNum = 1 - shop.skuNum;
      }
      try {
    ...
    async updateChenck(shop) {
      try {
        await this.$store.dispatch("updateChenck", {
          skuId: shop.skuId,
          isChecked:
            shop.isChecked === 1 ? (shop.isChecked = 0) : (shop.isChecked = 1),
        });
      } catch (error) {
        alert(error.messgee);
      }
